...
if (profile === "mtx-sidecar") {
    ...
    deploymentService.after('subscribe', async (_, req) => {
        ...
        try {
            ...
            tenantStore.set(tenant, { subdomain: subdomain });
        } catch (err) {
    ...
    deploymentService.after('unsubscribe', async (_, req) => {
        ...
        try {
            const tenantInfo = tenantStore.get(tenant);
            if (!tenantInfo || !tenantInfo.subdomain) {
                return;
            }
            const { subdomain } = tenantInfo;
            const { repositoryId } = getConfigurations();
            if (!repositoryId) {
                return;
            }
            ...
            if (!repoToOffboard) {
                console.warn(`SDM Plugin: Could not find a repository with externalId '${repositoryId}' for tenant ${tenant}.`);
            } else {
