...
async function updateLinkInDraft(req, data) {
  // Begin transaction
    const tx = cds.tx(req);
    try {
      // Insert data into the draft table equivalent to INSERT in Java
      const result = await tx.run(INSERT.into(req.target.name).entries(data));
      await tx.commit();

      // Result processing can be done here, like notifying the user or further data manipulation
      req.notify(200, 'Draft entry created successfully');
      return result;
    } catch (err) {
      // Error Handling
      await tx.rollback();
      req.error(500, 'Failed to create draft entry');
    }
}
