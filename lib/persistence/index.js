...
async function getDraftAttachmentsForUpID(attachments, req, repositoryId) {
  ...
  return await SELECT("filename", "mimeType", "content", "url", "ID", "HasActiveEntity", "note")
    .from(attachments)
    .where(conditions)
}
...
async function updateAttachmentInDraft(req, data) {
  ...
  LOG.debug(`[DEBUG] [persistence] updateAttachmentInDraft attachmentID=${attachmentID} url=${data.url}`);
...
async function updateLinkInDraft(req, data) {
  try {
    LOG.debug(`[DEBUG] [persistence] updateLinkInDraft inserting link entry`);
    // Use cds.db to execute the query instead of req.run
    await INSERT.into(req.target.name).entries(data);
  } catch (err) {
    LOG.error(`[ERROR] [persistence] updateLinkInDraft failed: ${err.message}`);
...
async function editLinkInDraft(req, data) {
  try {
    ...
    LOG.debug(`[DEBUG] [persistence] editLinkInDraft ID=${data.ID} linkUrl=${data.linkUrl}`);
    await UPDATE(req.target).set(updateData).where({ ID: data.ID });
  } catch (err) {
    LOG.error(`[ERROR] [persistence] editLinkInDraft failed: ${err.message}`);
    if (req.error) {
...
async function setRepositoryId(attachments, repositoryId) {
  if(attachments) {
    ...
    LOG.info(`[INFO] [persistence] setRepositoryId Updating ${nullAttachments.length} attachment(s) with repositoryId=${repositoryId}`);
