...
const ORPHAN_ENTITY = 'sap.sdm.OrphanCleanupQueue';
...
// ---------------------------------------------------------------------------
// Orphan Cleanup Queue helpers
// ---------------------------------------------------------------------------

/**
 * Record an SDM objectId that may need cleanup if the upload fails.
 * Called immediately after createEmptyDocument, before any chunk is appended.
 */
async function enqueueOrphan(objectId, repositoryId, filename) {
  try {
    await INSERT.into(ORPHAN_ENTITY).entries({ objectId, repositoryId, filename });
    console.log(`[orphan-queue] Enqueued objectId=${objectId} filename="${filename}"`);
  } catch (err) {
    // Non-fatal: orphan queue is best-effort; log and continue
    console.error(`[orphan-queue] Failed to enqueue objectId=${objectId}: ${err.message}`);
  }
}

/**
 * Remove a successfully-uploaded (or already-deleted) objectId from the queue.
 */
async function dequeueOrphan(objectId) {
  try {
    await DELETE.from(ORPHAN_ENTITY).where({ objectId });
    console.log(`[orphan-queue] Dequeued objectId=${objectId}`);
  } catch (err) {
    console.error(`[orphan-queue] Failed to dequeue objectId=${objectId}: ${err.message}`);
  }
}

/**
 * Return all pending orphan entries for the reconciliation job.
 */
async function getAllOrphans() {
  try {
    return await SELECT.from(ORPHAN_ENTITY);
  } catch (err) {
    console.error(`[orphan-queue] Failed to read orphan queue: ${err.message}`);
    return [];
  }
}
