...
class ReadAheadStream extends EventEmitter {
  ...
  _preloadChunks() {
    this.readPromise = (async () => {
      try {
        ...
        // Only emit 'error' if there is a listener; emitting with no listener crashes the process.
        // Client-disconnect errors are expected and handled by the upload caller via readError.
  ...
  /**
   * Read a full chunk (up to chunkSize bytes) from the stream with exponential
   * backoff retry for transient read errors (EOFException, InsufficientData).
   */
   ...
   * Returns bytes read (>0), 0 (no data yet), or -1 (EOF).
  ...
  async _readFromStream(stream, buffer, offset, length) {
    return new Promise((resolve, reject) => {
      ...
      if (chunk === null) {
        ...
        const onReadable = () => {
          cleanup();
          // Try exact length first; if not enough is buffered, take whatever is available
          let newChunk = stream.read(length);
          if (newChunk === null) {
            newChunk = stream.read();
          }
