...
let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async getParentId(attachments, req, upId){
    ...
    const metadata = await cds.ql.SELECT.one.from(req.target).where({ ID: attachmentID });
    let folderPath = metadata?.sdmPath;
    
    let parentId = null;

    if(!folderPath) folderPath = req.data?.sdmPath;
    
    if (folderPath) { 
      parentId = await getFolderIdByDynamicPath(repositoryId, folderPath, this.creds, destination);
      return parentId ?? (await createNestedFolders(repositoryId, folderPath, this.creds, destination));
    }
