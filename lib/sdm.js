...
module.exports = class SDMAttachmentsService extends (
  ...
  async handleEditLinkAction(req) {
    ...
    if (!existingAttachment || !existingAttachment.url) {
      req.reject(404, "Link to be edited not found or is invalid.");
      return;
    }
    ...
    try {
      ...
      await editLinkInDraft(req, updatedFields);
        ...
        const message = response?.response?.data?.message || "Failed to update the link in the repository.";
        req.reject(response.status || 500, message);
      }
    } catch (e) {
      req.reject(400, "call to editLink failed with an exception", e);
    }
  }
