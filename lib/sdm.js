...
// plugin works regardless of which version actually got installed.
function resolveAttachmentsBasic() {
  try {
    return require("@cap-js/attachments/srv/attachments/basic");
  } catch {
    return require("@cap-js/attachments/srv/basic");
  }
}

const FILE_SIZE_THRESHOLD = 400 * 1024 * 1024; // 400 MB — files above this use the chunked upload path

module.exports = class SDMAttachmentsService extends resolveAttachmentsBasic() {
  async init() {
    this.creds = this.options.credentials;
    this.originalUrlMap = new Map();
    LOG.info('[INFO] [SDMAttachmentsService] init - service initialized');
    return super.init();
  }
...
let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async getDestination(req, attachmentsEntity) {
    // Cache destinations per-composition on the request: a parent with two
    // attachment compositions (one annotated, one not) needs both flavors,
    // and the non-rename callsites still hit '_default' for back-compat.
    const cacheKey = attachmentsEntity?.name || '_default';
    if (req._sdmDestinations?.[cacheKey]) {
      LOG.debug(`[DEBUG] [getDestination] Using cached destination for key=${cacheKey}`);
      return req._sdmDestinations[cacheKey];
    }

    const userJwt =  retrieveJwt(req);
    let destination;
    if (isClientCredentialForced(req, attachmentsEntity) || !cds.context?.user?.authInfo?.token?.payload?.origin) {
      LOG.debug(`[DEBUG] [getDestination] Using client credentials flow for key=${cacheKey}`);
      let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
      ...
      LOG.debug(`[DEBUG] [getDestination] Using JWT bearer flow for key=${cacheKey}`);
      destination = await getDestinationFromServiceBinding({
        destinationName: getSdmInstanceName(),
        jwt: userJwt,
        useCache: true,
    ...
    if (!req._sdmDestinations) req._sdmDestinations = {};
    req._sdmDestinations[cacheKey] = destination;
    return destination;
  }
  getSDMCredentials() {
    return this.creds;
  ...
  async checkRepositoryType(req) {
    ...
    let isVersioned;
    let repoInfo;
    LOG.debug(`[DEBUG] [checkRepositoryType] repositoryId=${repositoryId} subdomain=${subdomain} cached=${repotype !== undefined}`);
    ...
    if (isVersioned) {
      LOG.warn(`[WARN] [checkRepositoryType] Repository ${repositoryId} is versioned - rejecting operation`);
  ...
  async get(attachments, keys) {
    ...
    LOG.debug(`[DEBUG] [get] Fetching attachment content objectId=${Key}`);
    // Access current request from cds.context for cloud-sdk authentication
    const req = cds.context?.http?.req;
    if (!req) {
      LOG.error('[ERROR] [get] HTTP request context not available');
      throw new Error('HTTP request context not available');
    }
    const destination = await this.getDestination(req, attachments);
  ...
  async updateDraftAttachments(req, attachment, attachmentsEntity, secondaryPropertiesWithInvalidDefinitions, secondaryTypeProperties, compositionName) {
    LOG.debug(`[DEBUG] [updateDraftAttachments] ID=${attachment.ID} composition=${compositionName}`);
    const attachmentData = await this.getAttachementDataInSDM(this.creds.uri, attachment.url, req, attachmentsEntity);
  ...
  async updateNonDraftAttachments(req, attachment, attachmentsEntity, secondaryPropertiesWithInvalidDefinitions, secondaryTypeProperties, compositionName) {
    LOG.debug(`[DEBUG] [updateNonDraftAttachments] ID=${attachment.ID} composition=${compositionName}`);
  ...
  async _updateAttachments(req, context) {
    ...
    if (Object.keys(updatedSecondaryProperties).length > 0) {
      const updateResult = await this.performAttachmentUpdate(
        req, attachment, updatedSecondaryProperties, invalidDefinitions, filenameInRequest, attachmentsEntity
      );
  ...
  async performAttachmentUpdate(req, attachment, updatedSecondaryProperties, invalidDefinitions, filenameInRequest, attachmentsEntity) {
    LOG.debug(`[DEBUG] [performAttachmentUpdate] filename=${filenameInRequest} objectId=${attachment.url} properties=${Object.keys(updatedSecondaryProperties).join(',')}`);
    try {
      const destination = await this.getDestination(req, attachmentsEntity);
      ...
      switch (responseCode) {
        case 403:
          LOG.warn(`[WARN] [performAttachmentUpdate] No SDM roles for filename=${filenameInRequest}`);
          return { error: { typeOfError: 'no sdm roles', name: filenameInRequest } };
        case 409:
          LOG.warn(`[WARN] [performAttachmentUpdate] Duplicate filename=${filenameInRequest}`);
          return { error: { typeOfError: 'duplicate', name: filenameInRequest } };
        case 404:
          LOG.warn(`[WARN] [performAttachmentUpdate] Not found filename=${filenameInRequest}`);
          return { error: { typeOfError: 'not found', name: filenameInRequest } };
        case 200:
        case 201:
          LOG.info(`[INFO] [performAttachmentUpdate] Success filename=${filenameInRequest}`);
          return { success: true };
        default:
      ...
      if (e.message.startsWith(unsupportedProperties)) {
        ...
        LOG.warn(`[WARN] [performAttachmentUpdate] Unsupported properties: ${unsupportedDetails}`);
        return { error: { typeOfError: 'unsupported properties', details: unsupportedDetails } };
      }
      LOG.error(`[ERROR] [performAttachmentUpdate] Failed filename=${filenameInRequest} error=${e.message}`);
  ...
  async getAttachementDataInSDM(uri, objectId, req, attachmentsEntity) {
    const destination = await this.getDestination(req, attachmentsEntity);
   ...
   */
  _rejectIfVirusScanLargeFile(req, contentLengthNum) {
    if (contentLengthNum <= FILE_SIZE_THRESHOLD) return;
    ...
    if (isVirusScanEnabled === 'true' || isVirusScanEnabled === true) {
      LOG.warn(`[WARN] [draftAttachmentUploadHandler] Rejecting: file size ${contentLengthNum} exceeds 400MB limit for virus scan enabled repository`);
    ...
    if (req?.data?.content) {
      ...
      if (attachment_val.length > 0) {
        ...
        if (attachment_val_create.length > 0) {
          ...
          LOG.info(`[INFO] [draftAttachmentUploadHandler] Upload finished`);
        }
      }
  ...
  async create(attachment_val_create, attachments, req){
    ...
    LOG.debug(`[DEBUG] [create] Resolved parent folder ID ${parentId}`);
    await this.onCreate(
  ...
  /**
   * Returns true when the entity owning `attachments` has more than one
   * Composition of Attachments defined in the CDS model.
   * @private
   */
  isMultiCompositionEntity(attachments) {
    // Entity name pattern: "Srv.ParentEntity.compositionName"
    const parts = attachments.name.replace(/\.drafts$/, '').split('.');
    if (parts.length < 2) return false;
    const parentEntityName = parts.slice(0, -1).join('.');
    const parentEntity = cds.model.definitions[parentEntityName];
    if (!parentEntity) return false;
    return this.getAttachmentCompositions(parentEntity).length > 1;
  }

  /**
   * Derives the composition name from the attachment entity name.
   * e.g. "ProcessorService.Incidents.references" → "references"
   * @private
   */
  getCompositionName(attachments) {
    return attachments.name.replace(/\.drafts$/, '').split('.').pop();
  }

  /**
   * For multi-composition entities, creates/finds a folder named {entityId}_{compositionName}
   * directly under /root/. This ensures each composition section has its own isolated folder.
   * @private
   */
  async getOrCreateCompositionFolder(attachments, req, upId, compositionName) {
    const { repositoryId } = getConfigurations();
    const up_ = attachments.keys.up_.keys[0].$generatedFieldName;
    const entityId = req.data[up_] || upId;
    const composedFolderName = `${entityId}_${compositionName}`;
    ...
    // Try to find the folder in SDM by path /root/{entityId}_{compositionName}
    ...
    try {
      const response = await executeHttpRequest(
        destination, { method: 'GET', url: getFolderByPathURL }
      );
      return response.data.properties["cmis:objectId"].value;
    } catch {
      // Folder doesn't exist yet — create it below
    }

    const originalUpValue = req.data[up_];
    req.data[up_] = composedFolderName;
    ...
    if (response.status == 403 && response.response?.data == userDoesNotHaveRequiredScope) {
      LOG.error('[ERROR] [getOrCreateCompositionFolder] User not authorized to create composition folder');
      req.reject(403, userNotAuthorisedError);
    }
    // If folder creation failed (e.g., 409 conflict), try to fetch it again
    try {
      const retryResponse = await executeHttpRequest(
        destination, { method: 'GET', url: getFolderByPathURL }
      );
      return retryResponse.data.properties["cmis:objectId"].value;
    } catch {
      LOG.error('[ERROR] [getOrCreateCompositionFolder] Failed to create or find folder', {
        composedFolderName,
        status: response.status || response.response?.status,
        message: response.message || response.response?.data?.message
      });
      req.reject(500, `Failed to create folder for composition: ${compositionName}`);
    }
  }

  async getParentId(attachments, req, upId) {
    if (this.isMultiCompositionEntity(attachments)) {
      const compositionName = this.getCompositionName(attachments);
      return this.getOrCreateCompositionFolder(attachments, req, upId, compositionName);
    }
    return this.getOrCreateFlatFolder(attachments, req, upId);
  }

  /**
   * folder strategy: /{entityId}
   * Used when an entity has exactly one Composition of Attachments.
   * @private
   */
  async getOrCreateFlatFolder(attachments, req, upId) {
    const { repositoryId } = getConfigurations();
    const folderIds = await getFolderIdForEntity(attachments, req, repositoryId, upId);
    let parentId = null;
    ...
    LOG.debug(`[DEBUG] [getOrCreateFlatFolder] repositoryId=${repositoryId} cachedFolderId=${parentId || 'none'}`);
    if (!parentId) {
      const destination = await this.getDestination(req, attachments);
      ...
      if (folderId) {
        ...
        if (response.status == 403 && response.response.data == userDoesNotHaveRequiredScope) {
          LOG.error('[ERROR] [getParentId] User not authorized to create folder');
          req.reject(403, userNotAuthorisedError);
        }
  ...
  async isFileNameDuplicateInDrafts(data, req) {
    ...
    if (duplicates.length != 0) {
      LOG.warn(`[WARN] [isFileNameDuplicateInDrafts] Duplicate filenames detected: ${duplicates.join(', ')}`);
  ...
  async validateLinkName(data, linkNameInRequest, req) {
    ...
    if (nameConstraint) {
      LOG.warn(`[WARN] [validateLinkName] Restricted characters in link name: ${linkNameInRequest}`);
    ...
    const duplicates = this.filterDuplicates(fileNames);
    if (duplicates.length != 0) {
      LOG.warn(`[WARN] [validateLinkName] Duplicate link name detected: ${duplicates.join(', ')}`);
      req.reject(409, duplicateDraftFileErr(duplicates.join(", ")));
    }
  }
  ...
  async attachDeletionData(req) {
    ...
    const attachmentCompositions = this.getAttachmentCompositions(req.target);
    LOG.debug(`[DEBUG] [attachDeletionData] entity=${req.target.name} event=${req.event} compositions=${attachmentCompositions.length}`);
  ...
  async addFolderToParentIdList(req, attachments) {
    ...
    if (this.isMultiCompositionEntity(attachments)) {
      // For multi-composition entities, look for /root/{entityId}_{compositionName}
      const compositionName = this.getCompositionName(attachments);
      const up_ = attachments.keys.up_.keys[0].$generatedFieldName;
      const idValue = up_.split("__")[1];
      const entityId = req.data[idValue];
      if (!entityId) return;
      const composedFolderName = `${entityId}_${compositionName}`;
      const { repositoryId } = getConfigurations();
      const getFolderByPathURL =
        this.creds.uri + "browser/" + repositoryId + "/root/" +
        composedFolderName + "?cmisselector=object";
      try {
        const response = await executeHttpRequest(
          destination, { method: 'GET', url: getFolderByPathURL }
        );
        const folderId = response.data.properties["cmis:objectId"].value;
        if (folderId) {
  ...
  async attachDraftDeletionData(req) {
    ...
    LOG.debug(`[DEBUG] [attachDraftDeletionData] entity=${baseEntityName} event=${req.event}`);
    const attachmentCompositions = this.getAttachmentCompositions({ name: baseEntityName });
    const diffData = req.event == "DELETE" ? await req.diff() : null;

    for (const compositionName of attachmentCompositions) {
      const draftAttachments = this.getDraftAttachmentsEntity(baseEntityName, compositionName);
      if (!draftAttachments) {
        continue;
      }

      const attachmentsToDeleteFromDraft = await this.collectDraftAttachmentDeletionUrls(req, draftAttachments);
      if (!this.shouldCollectDraftParentFolder(req, diffData, compositionName, attachmentsToDeleteFromDraft)) {
        continue;
      }

      const folderId = await this.resolveDraftParentFolderId(req, draftAttachments, compositionName);
      this.appendParentId(req, folderId);
    }
  }

  getDraftAttachmentsEntity(baseEntityName, compositionName) {
    return cds.model.definitions[baseEntityName + "." + compositionName + ".drafts"];
  }

  async collectDraftAttachmentDeletionUrls(req, draftAttachments) {
    ...
    return attachmentsToDeleteFromDraft;
  }

  shouldCollectDraftParentFolder(req, diffData, compositionName, attachmentsToDeleteFromDraft) {
    return req.event == "DELETE" && diffData?.[compositionName]?.length == attachmentsToDeleteFromDraft?.length;
  }

  async resolveDraftParentFolderId(req, draftAttachments, compositionName) {
    const destination = await this.getDestination(req);
    if (this.isMultiCompositionEntity(draftAttachments)) {
      return this.getComposedDraftFolderId(req, draftAttachments, compositionName, destination);
    }
    return getFolderIdByIDAsPath(
      req,
      this.creds,
      destination,
      draftAttachments
  ...
  async getComposedDraftFolderId(req, draftAttachments, compositionName, destination) {
  ...
  async attachURLsToDeleteFromAttachmentsDraft(req) {
    ...
    if(draftAttachments)  {
      ...
      if (attachmentsToDeleteFromDraft?.length > 0) {
        LOG.info(`[INFO] [attachURLsToDeleteFromAttachmentsDraft] Marking ${attachmentsToDeleteFromDraft.length} draft attachment(s) for deletion ID=${req.data.ID}`);
  ...
  async deleteAttachmentsWithKeys(records, req) {
    let failedReq = [],
      Ids = [];
    if (req?.attachmentsToDelete?.length > 0) {
      LOG.info(`[INFO] [deleteAttachmentsWithKeys] Deleting ${req.attachmentsToDelete.length} attachment(s)`);
       if (req?.parentId) {
    ...
    await Promise.all(
      data.map(async (d) => {
        ...
        try {
          ...
          LOG.error('[ERROR] [onCreate] createAttachment threw error', {
            filename: d.filename,
        ...
        if (response.status == 201) {
          ...
          if (req.target.isDraft) {
            ...
            LOG.warn('[WARN] [onCreate] No update performed - missing ID', {
              filename: d.filename,
              isDraft: req.target.isDraft
            });
          }
        } else {
          LOG.error('[ERROR] [onCreate] Upload failed', {
            filename: d.filename,
          ...
          if (!req.target.isDraft && d.ID) {
            try {
              ...
              LOG.error('[ERROR] [onCreate] Cleanup failed', {
                ID: d.ID,
          ...
          if(response.response.data.message == 'Malware Service Exception: Virus found in the file!'){
            LOG.warn('[WARN] [onCreate] Virus detected', { fileNames });
            ...
            LOG.warn('[WARN] [onCreate] Duplicate file detected', { 
              fileNames,
            ...
            LOG.warn('[WARN] [onCreate] Blocked MIME type', { fileNames, message: response.response?.data?.message });
            req.reject(403, mimeTypeInvalidError);
          }
          else if(response.status == 403){
            LOG.warn('[WARN] [onCreate] User not authorized', { fileNames });
             req.reject(403, userNotAuthorisedError);
          }
          else{
            LOG.error('[ERROR] [onCreate] Other upload error', { 
              fileNames,
              status: response.status,
  ...
  async openAttachment(req) {
    ...
    LOG.debug(`[DEBUG] [openAttachment] attachmentId=${attachmentId.ID}`);
    ...
    if (response?.mimeType.toLowerCase() == "application/internet-shortcut") {
      LOG.info(`[INFO] [openAttachment] Opening link attachment ID=${attachmentId.ID} linkUrl=${response.linkUrl}`);
    ...
    req.reject(403,userNotAuthorisedOpenLink);
      ...
      LOG.debug(`[DEBUG] [openAttachment] Non-link attachment ID=${attachmentId.ID} mimeType=${response?.mimeType}`);
      return { value: "None" };
    }
  }
  ...
  async handleCreateLinkAction(req) {
    ...
    LOG.info(`[INFO] [createLink] action called`, {
      repositoryId,
  ...
  async processLinkCreation(linkToCreateInSDM, attachment, req) {
    ...
    LOG.info(`[INFO] [processLinkCreation] called`, {
      upIdKey,
      upId,
      linkToCreateInSDM
    });
    ...
    LOG.info(`[INFO] [processLinkCreation] createLink completed`, {
      parentId,
      upIdKey,
      upId
    });
  ...
  async createLink(linkToCreateInSDM, credentials, req, parentId, upIdKey) {
    ...
    LOG.info(`[INFO] [createLink] called`, {
      linkToCreateInSDM,
      parentId,
      upIdKey,
      upId
    ...
    LOG.info(`[INFO] [createLink] createAttachment response`, {
      status: response.status
    });
    ...
    if (response.status == 201) {
      ...
      LOG.info(`[INFO] [createLink] draftUUID fetched`, {
      ...
      // Custom action bypasses srv.before hooks, so stamp the SDM technical
      // user directly into the draft row when the annotation is in effect.
      // Without this, CAP's managed aspect would persist req.user.id and the
      // UI would show the human name even though DI shows the client_id.
      if (isClientCredentialForced(req)) {
        const clientId = getSdmClientId();
        if (clientId) this._setManagedUser(updatedFields, 'CREATE', clientId);
      }

      LOG.info(`[INFO] [createLink] updating link in draft`, { updatedFields });
      ...
      if (response.response.data.exception == "nameConstraintViolation") {
        LOG.warn(`[WARN] [createLink] nameConstraintViolation`, { fileName, response: response.response.data });
        req.reject(409, duplicateFileErr([fileName]));
      } else if (response.status == 403) {
        LOG.warn(`[WARN] [createLink] user not authorised`, { user: req.user?.id, response: response.response.data });
        req.reject(403, userNotAuthorisedErrorLink);
      }
      else {
        LOG.error(`[ERROR] [createLink] other error`, { message: response?.response?.data?.message, response: response.response.data });
  ...
  async handleEditLinkAction(req) {
    ...
    LOG.info(`[INFO] [handleEditLinkAction] attachmentId=${attachmentId} found=${!!existingAttachment}`);

    if (!existingAttachment || !existingAttachment.url) {
      LOG.warn(`[WARN] [handleEditLinkAction] Link not found for attachmentId=${attachmentId}`);
    ...
    if (status === 200 || status === 201) {
      ...
      // Custom action bypasses srv.before hooks, so stamp the SDM technical
      // user directly. UPDATE event => modifiedBy only.
      if (isClientCredentialForced(req)) {
        const clientId = getSdmClientId();
        if (clientId) this._setManagedUser(updatedFields, 'UPDATE', clientId);
      }
      ...
      LOG.warn(`[WARN] [editLink] user not authorised`, { user: req.user?.id, response: response?.response?.data });
      req.reject(400, userNotAuthorisedErrorEditLink);
    } else {
      LOG.error(`[ERROR] [editLink] other error`, { message: response?.response?.data?.message, response: response.response.data });
      req.reject(response?.response?.data?.message);
    }
  }
  ...
  async handleDraftSaveForLinks(data, req) {
    ...
    const clientId = getSdmClientId();
    LOG.debug(`[DEBUG] [handleDraftSaveForLinks] entity=${baseEntityName} parentId=${parentId} compositions=${attachmentCompositions.length}`);
    ...
    for (const compositionName of attachmentCompositions) {
      ...
      // After draft activation, fix up createdBy/modifiedBy on the active
      // table:
      //   - Freshly activated rows (new uploads / new links): set BOTH
      //     createdBy and modifiedBy to client_id. createdBy is set here once
      //     and never touched again.
      //   - Pre-existing rows whose draft entry was edited under client
      //     credentials in this session: set ONLY modifiedBy to client_id.
      //     createdBy is intentionally left as-is.
      // Untouched rows (in draft only because the draft was opened, not
      // edited) are filtered out by the draft.modifiedBy === client_id check
      // captured in the snapshot.
      if (
        clientId &&
        parentId &&
        attachmentsEntity["@SDM.useClientCredential"] === true &&
        req._sdmSaveSnapshot?.[compositionName]
      ) {
        const { preExistingActiveIds, draftClientStampedIds } =
          req._sdmSaveSnapshot[compositionName];
        const draftIds = [...draftClientStampedIds];
        const freshlyActivated = draftIds.filter((id) => !preExistingActiveIds.has(id));
        const modifiedExisting = draftIds.filter((id) => preExistingActiveIds.has(id));

        if (freshlyActivated.length > 0) {
          await global.UPDATE(attachmentsEntityName)
            .set({ createdBy: clientId, modifiedBy: clientId })
            .where({ ID: { in: freshlyActivated } });
        }
        if (modifiedExisting.length > 0) {
          await global.UPDATE(attachmentsEntityName)
            .set({ modifiedBy: clientId })
            .where({ ID: { in: modifiedExisting } });
        }
      }
  ...
  /**
   * Snapshot two ID sets per annotated attachment composition before draft
   * activation runs:
   *   1. preExistingActiveIds — rows already on the active table for this
   *      parent. Used to distinguish freshly activated rows from edits to
   *      pre-existing rows.
   *   2. draftClientStampedIds — rows in the draft table whose modifiedBy is
   *      client_id, i.e. the user actually touched them in this draft session
   *      under @SDM.useClientCredential. Untouched draft rows (the draft was
   *      opened but the row wasn't edited) are excluded so we don't bump
   *      modifiedBy on rows the user only looked at.
   * Stored on req._sdmSaveSnapshot[compositionName]. Only populated for
   * compositions whose target carries @SDM.useClientCredential: true.
   */
  async captureSaveSnapshot(req) {
    const clientId = getSdmClientId();
    if (!clientId) return;
    const baseEntityName = req.target.name;
    const parentId = req.data?.ID;
    ...
    const attachmentCompositions = this.getAttachmentCompositions({ name: baseEntityName });
    if (attachmentCompositions.length === 0) return;
    ...
    for (const compositionName of attachmentCompositions) {
      ...
      const attachmentsEntity = cds.model.definitions[attachmentsEntityName];
      if (!attachmentsEntity) continue;
      if (attachmentsEntity["@SDM.useClientCredential"] !== true) continue;

      const upKey = attachmentsEntity.keys?.up_?.keys?.[0]?.$generatedFieldName || 'up__ID';
      const draftEntityName = `${attachmentsEntityName}.drafts`;

      const activeRows = await global.SELECT.from(attachmentsEntityName)
        .columns('ID')
        .where({ [upKey]: parentId });

      const draftRows = cds.model.definitions[draftEntityName]
        ? await global.SELECT.from(draftEntityName)
            .columns('ID')
            .where({ [upKey]: parentId, modifiedBy: clientId })
        : [];

      req._sdmSaveSnapshot[compositionName] = {
        preExistingActiveIds: new Set(activeRows.map((r) => r.ID)),
        draftClientStampedIds: new Set(draftRows.map((r) => r.ID))
      };
    }
  }
      ...
      for (const attachment of draftAttachments) {
        if (attachment.note?.startsWith("__BASELINE_URL__:")) {
          ...
          if (baselineUrl && attachment.linkUrl !== baselineUrl) {
            LOG.info(`[INFO] [handleDraftDiscardForLinks] Reverting link ID=${attachment.ID} to baseline URL`);
            await this.revertLinkInSDM(attachment, baselineUrl, req, attachmentsEntity);
  ...
  async revertLinkInSDM(draftAttachment, originalLinkUrl, req, attachmentsEntity) {
    try {
      ...
      LOG.error(`[FATAL] [revertLinkInSDM] error reverting link for attachment ${draftAttachment.ID}:`, error.message);
      throw error;
    }
  }
   ...
   * @private
   */
  async _updateAttachmentInSDM(req, currentAttachment, attachment, updatedProperties, invalidDefinitions, filename, attachmentsEntity) {
    try {
      const destination = await this.getDestination(req, attachmentsEntity);
  ...
  async attachNonDraftAttachmentDeletionData(req) {
    ...
    if (attachments.length) {
      LOG.info(`[INFO] [attachNonDraftAttachmentDeletionData] Marking ${attachments.length} attachment(s) for deletion`);
      req.attachmentsToDelete = attachments.map(a => ({ ...a, target: req.target.name }));
    }
  }

  /**
   * When @SDM.useClientCredential is set on the attachment target, the file is
   * uploaded to SDM under the technical user (UAA clientid). To keep the plugin
   * DB consistent with what DMS/DI shows, we override createdBy/modifiedBy on
   * the request data so CAP's managed aspect doesn't fall back to the human
   * user. Runs as a before-hook for CREATE/UPDATE/PUT on the attachment entity
   * (and its drafts), and for SAVE on the parent entity (where we have to walk
   * the attachment compositions, since the annotation lives on the target).
   */
  applyClientCredentialUser(req) {
    const clientId = getSdmClientId();
    if (!clientId) return;

    // Direct attachment-target operations: req.target carries the annotation.
    if (isClientCredentialForced(req)) {
      this._setManagedUser(req.data, req.event, clientId);
      return;
    }

    // Parent SAVE: annotation lives on the composition target, not on req.target.
    if (req.event !== 'SAVE' && req.event !== 'CREATE' && req.event !== 'UPDATE') return;
    if (!req.target?.elements) return;

    for (const [elementName, element] of Object.entries(req.target.elements)) {
      if (element.type !== 'cds.Composition' || !element.target) continue;
      const targetDef = cds.model.definitions[element.target];
      if (!targetDef?.["@SDM.useClientCredential"]) continue;
      const rows = req.data?.[elementName];
      if (!Array.isArray(rows)) continue;
      for (const row of rows) {
        // Skip rows the user did not touch:
        //   - 'delete' rows shouldn't have managed fields written
        //   - 'read'   rows are included by CAP for reference / unchanged children;
        //              stamping modifiedBy here would falsely mark them as edited
        if (row?._op === 'delete' || row?._op === 'read') continue;
        this._setManagedUser(row, row?._op === 'update' ? 'UPDATE' : 'CREATE', clientId);
      }
    }
  }

  _setManagedUser(data, event, clientId) {
    if (!data || typeof data !== 'object') return;
    if (event === 'CREATE' || event === 'PUT') {
      data.createdBy = clientId;
    }
  ...
  registerHandlers(srv) {
    ...
    LOG.info(`[INFO] [registerHandlers] Registering SDM handlers for service=${srv.name}`);
    // Prevent duplicate registrations when one entity has multiple attachment compositions
    this._registeredEntityHandlers = this._registeredEntityHandlers || new Set();
    this._registeredTargetHandlers = this._registeredTargetHandlers || new Set();
    this._registeredGlobalActionHandlers = this._registeredGlobalActionHandlers || false;
  ...
  registerSDMHandlers(srv, entity, target) {
    ...
    const applyClientCred = this.applyClientCredentialUser.bind(this);
    srv.before(["CREATE", "UPDATE", "PUT"], target, applyClientCred);
    if (target.drafts) {
      srv.before(["CREATE", "UPDATE", "PUT"], target.drafts, applyClientCred);
    }
    if (entity.drafts) {
      // Draft activation: annotation lives on the composition target, so we
      // hook on the parent SAVE and walk compositions inside the handler.
      srv.before("SAVE", entity, applyClientCred);
    } else {
      // Non-draft parent UPDATE/CREATE may carry composition rows too.
      srv.before(["CREATE", "UPDATE"], entity, applyClientCred);
    }

    const entityKey = entity.name;
    const targetKey = target.name;

    // Register entity-level handlers exactly once per parent entity
    if (!this._registeredEntityHandlers.has(entityKey)) {
      this._registeredEntityHandlers.add(entityKey);
    ...
    // Register target-level handlers once per attachment composition entity
    if (!this._registeredTargetHandlers.has(targetKey)) {
      this._registeredTargetHandlers.add(targetKey);
    ...
    // Register service-level action handlers once
    if (!this._registeredGlobalActionHandlers) {
      this._registeredGlobalActionHandlers = true;
