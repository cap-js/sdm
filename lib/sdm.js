...
module.exports = class SDMAttachmentsService extends (
  ...
  async draftSaveHandler(req) {
    ...
    if (attachment_val.length > 0) {
      const duplicateDraftFilesErrMsg = this.isFileNameDuplicateInDrafts(attachment_val);
      ...
      attachment_val_rename.forEach(draftAttachment => {
        ...
        if(correspondingAttachment && correspondingAttachment.filename !== draftAttachment.filename) {
          modifiedAttachments.push({url: draftAttachment.url, name: draftAttachment.filename, prevname: correspondingAttachment.filename});
        }
      });
  ...
  async onRename(modifiedAttachments, credentials, token) {
    ...
    await Promise.all(
      modifiedAttachments.map(async (a) => {
          let oldName = a.prevname;
          const response = await renameAttachment(a, credentials, token);
          if (response.status != 200) {
              failedReq.push(response.response.data.message);
              failedNames.push(oldName); // If the rename fails, push the old name to failedNames
          }
          else {
              success.push(a);   
              successNames.push(a.name); // If the rename is successful, push the new name to successNames
          }
      })
    );
  
    let removeFailedNames = obj => failedNames.includes(obj.prevname);
    let removeSuccessNames = obj => successNames.includes(obj.name);
  
    // Filter out attachments that failed to rename
    modifiedAttachments = modifiedAttachments.filter(obj => !removeFailedNames(obj));
    
    // Filter out attachments that were successfully renamed
    modifiedAttachments = modifiedAttachments.filter(obj => !removeSuccessNames(obj));
  
    // Append successful attachments to the end of the array
    modifiedAttachments = [...modifiedAttachments, ...success];
