 ...
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async getOrCreateCompositionFolder(attachments, req, upId, compositionName) {
    ...
    if (response.status == 403 && response.response?.data == userDoesNotHaveRequiredScope) {
      console.error('[getOrCreateCompositionFolder] User not authorized to create composition folder');
    ...
    try {
      ...
      console.error('[getOrCreateCompositionFolder] Failed to create or find folder', {
        composedFolderName,
  ...
  async getParentId(attachments, req, upId) {
    // If the caller supplied a custom sdmPath, resolve or create that nested
    // folder hierarchy and use it as the parent — regardless of composition count.
    const sdmPath = req.data?.sdmPath;
    if (sdmPath) {
      const { repositoryId } = getConfigurations();
      const destination = await this.getDestination(req, attachments);

      // Try to find the leaf folder first (warm path — avoids redundant creates).
      const existingId = await getFolderIdByDynamicPath(repositoryId, sdmPath, this.creds, destination);
      if (existingId) {
        return existingId;
      }

      // Leaf doesn't exist yet — walk the path and create missing segments.
      return createNestedFolders(sdmPath, this.creds, repositoryId, destination);
    }
  ...
  /**
   * Returns true when every attachment row for this entity used a custom sdmPath.
   * In that case we must NOT call deleteTree because the folder may be shared
   * with other entity records. Individual file deletes are handled separately
   * via attachmentsToDelete.
   * @private
   */
  async _hasCustomPathAttachments(attachments, req) {
    try {
      const up_ = attachments.keys.up_.keys[0].$generatedFieldName;
      ...
      const rows = await cds.ql.SELECT.from(attachments)
        .columns("sdmPath")
        .where({ [up_]: entityId });

      // Only skip deleteTree when ALL rows carry a custom path — if even one
      // row used the default folder strategy we still own that folder.
      return rows.length > 0 && rows.every(r => r.sdmPath);
    } catch {
      return false;
    }
  }
  ...
  async addFolderToParentIdList(req, attachments) {
    ...
    // Custom-path attachments share their folder with potentially other entities.
    // Never deleteTree a shared folder — individual files are cleaned up via
    // req.attachmentsToDelete instead.
    if (await this._hasCustomPathAttachments(attachments, req)) {
      return;
    }
  ...
  async resolveDraftParentFolderId(req, draftAttachments, compositionName) {
    // Custom-path drafts must not trigger deleteTree — their folder may be
    // shared across entity records.
    if (await this._hasCustomPathAttachments(draftAttachments, req)) {
      return null;
    }
