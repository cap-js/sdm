...
let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async getParentId(attachments, req, upId){
    ...
    let parentId = null;
    const compositionName = this._getCompositionName(attachments.name);
  ...
  _getCompositionName(entityName) {
    if (!entityName) return null;
    const cleanName = entityName.replace(/\.drafts$/, "");
    const lastDot = cleanName.lastIndexOf(".");
    return lastDot >= 0 ? cleanName.substring(lastDot + 1) : null;
  }
  ...
  async processAttachmentDeletion(req, diffData, compositionName, attachments) {
    ...
    if (req.event === "DELETE") {
      await this.addFolderToParentIdList(req, attachments, compositionName);
    }
  ...
  async addFolderToParentIdList(req, attachments, compositionName) {
    const destination = await this.getDestination(req);
  ...
  async nonDraftEntityRenameHandler(req) {
    ...
    for (const compositionName of attachmentCompositions) {
      ...
      const updatedAttachments = await this._getUpdatedAttachmentsForComposition(req, compositionName);
      if (!updatedAttachments || updatedAttachments.length === 0) continue;
  ...
  /**
   * Extracts attachments that need to be updated from diff data for a specific composition
   * @private
   */
  async _getUpdatedAttachmentsForComposition(req, compositionName) {
    const diffData = await req.diff();
    if (!diffData?.[compositionName]?.length) return null;

    const updatedAttachments = diffData[compositionName].filter(att =>
   ...
   * @private
   * @deprecated Use _getUpdatedAttachmentsForComposition instead
   */
  async _getUpdatedAttachments(req) {
    return this._getUpdatedAttachmentsForComposition(req, 'attachments');
  }
