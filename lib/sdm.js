  ...
  async onCreate(data, credentials, token, attachments, req, parentId) {
    ...
    await Promise.all(
      data.map(async (d) => {
        ...
        if (d.content === null) {
              const streams = await SELECT("chunk").from("Chunks").where({ID: d.ID}).orderBy('ChunkID');
               if (!streams || streams.length === 0) {
                console.log('No data found for the given ID');
                return;
            }
            //streams is an array we need to club it and form the file again and add it to d.content
...
//do this only if file size greater that 20MB req.header.content-length(is in bytes)
    if (req._path.endsWith("content")) {
      const chunks = [];
      req.data.content.on('data', chunk => chunks.push(chunk));

      await new Promise((resolve, reject) => {
        req.data.content.on('end', resolve).on('error', reject);
      });
    let  i=1;
     for (let chunk of chunks) {
          await INSERT.into("Chunks")
          .entries({
            ChunkID: i,
              ID: req.data.ID,
              chunk: chunk
      ...
      console.log("Data processing has finished");
