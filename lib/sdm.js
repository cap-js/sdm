...
let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
 const technicalUserDestn = await getDestinationFromServiceBinding({
      ...
      data.map(async (d) => {
        ...
        if (response.status == 201) {
          ...
          if (req.target.isDraft) {
            ...
            try {
              const updateResult = await UPDATE(req.target)
                .set({ 
                  folderId: d.folderId, 
                  url: d.url, 
              ...
              const verifyRecord = await SELECT.one.from(req.target).where({ ID: d.ID });
            } catch (updateError) {
  ...
  async nonDraftAttachmentCreateHandler(req) {
    ...
    const updatedRecord = await SELECT.one.from(req.target).where({ ID: attachmentID });
    if (updatedRecord) {
