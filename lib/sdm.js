...
module.exports = class SDMAttachmentsService extends (
  ...
  async renameHandler(req) {
    ...
    // Get both draft and active attachments
    const draftAttachments = await getDraftAttachments(attachmentsEntity, req, repositoryId);
    const activeAttachments = await getActiveAttachments(attachmentsEntity, req.data.entityId, repositoryId);
    
    const allAttachments = [...draftAttachments, ...activeAttachments];
    ...
    if (allAttachments.length > 0) {
      ...
      await this.isFileNameDuplicateInDrafts(allAttachments, req);

      const propertyTitles = getPropertyTitles(attachmentsEntity, allAttachments[0]);
      const secondaryPropertiesWithInvalidDefinitions =
              getSecondaryPropertiesWithInvalidDefinition(
                attachmentsEntity, allAttachments[0]);
      
      const secondaryTypeProperties = getSecondaryTypeProperties(attachmentsEntity, allAttachments[0]);
      
      let attachment_val_rename = [];
      let draft_attachments = [];
      draft_attachments = allAttachments.filter(attachment => attachment.HasActiveEntity === false);
      attachment_val_rename = allAttachments.filter(attachment => attachment.HasActiveEntity === true);
  ...
  // Following CAP attachments pattern for non-draft operations
  async getAttachments(req) {
    const { entityId, draftsOnly } = req.data;
    const { repositoryId } = getConfigurations();
    const attachmentsEntity = cds.model.definitions[req.target.name];
    
    if (draftsOnly) {
      return await getDraftAttachments(attachmentsEntity, req, repositoryId);
    } else {
      // Get both draft and active attachments
      const draftAttachments = await getDraftAttachments(attachmentsEntity, req, repositoryId);
      const activeAttachments = await getActiveAttachments(attachmentsEntity, entityId, repositoryId);
      
      return [...draftAttachments, ...activeAttachments];
    }
  }

  async getAttachmentContent(req) {
    const { ID } = req.data;
    const attachmentsEntity = cds.model.definitions[req.target.name];
    
    // Try to get from active first, then draft
    let attachment = await getActiveAttachmentById(ID, attachmentsEntity);
    if (!attachment) {
      attachment = await getAttachmentById(ID, attachmentsEntity);
    }
    ...
    return await readAttachment(attachment.url, token, this.creds);
  }

  async uploadAttachment(req) {
    const { entityId, content, filename } = req.data;
    const { repositoryId } = getConfigurations();
    ...
    // Get parent folder
    const attachmentsEntity = cds.model.definitions[req.target.name];
    const parentId = await this.getParentId(attachmentsEntity, req, token);
    
    // Create attachment in SDM
    ...
    const response = await createAttachment(attachmentData, this.creds, token, parentId);
    
    if (response.status === 201) {
      // Save to database as active attachment
      ...
      return await createActiveAttachment(attachmentsEntity, activeAttachment);
    } else {
      this.handleUploadError(response, [filename], req);
    }
  }

  async updateAttachment(req) {
    const { ID, filename, note } = req.data;
    const attachmentsEntity = cds.model.definitions[req.target.name];
    
    // Check if it's active or draft attachment
    let attachment = await getActiveAttachmentById(ID, attachmentsEntity);
    const isActive = !!attachment;
    ...
    if (isActive) {
      return await updateActiveAttachment(attachmentsEntity, ID, updateData);
    } else {
      return await updateAttachmentInDraft(req, { ...updateData, ID });
    }
  }

  async deleteAttachment(req) {
    const { ID } = req.data;
    const attachmentsEntity = cds.model.definitions[req.target.name];
    ...
    // Delete from SDM
    if (attachment.url) {
      await deleteAttachmentsOfFolder(this.creds, token, attachment.url);
    }
    
    // Delete from database
    if (isActive) {
      return await deleteActiveAttachment(attachmentsEntity, ID);
    } else {
      // Handle draft deletion
      return await this.deleteDraftAttachment(attachmentsEntity, ID);
    }
  }

  async activateDraft(req) {
    const { entityId } = req.data;
    const { repositoryId } = getConfigurations();
    const draftAttachments = cds.model.definitions[req.target.name + ".drafts"];
    const activeAttachments = cds.model.definitions[req.target.name];
    
    // Get all draft attachments for entity
    const drafts = await getDraftAttachments(draftAttachments, req, repositoryId);
    
    // Move each draft to active
    for (const draft of drafts) {
      if (draft.entityId === entityId) {
        await moveAttachmentToActive(draftAttachments, activeAttachments, draft.ID);
      }
    }
  ...
  async discardDraft(req) {
    ...
    const draftIds = drafts
      .filter(draft => draft.entityId === entityId)
      .map(draft => draft.ID);
    
    // Delete all drafts
    return await this.bulkDeleteDrafts(draftAttachments, draftIds);
  }
  ...
  async bulkDelete(req) {
    ...
    return await bulkDeleteActiveAttachments(attachmentsEntity, IDs);
  }
  ...
  async getStatistics(req) {
    ...
    return await getAttachmentStatistics(attachmentsEntity, entityId, repositoryId);
  }
  ...
  registerUpdateHandlers(srv, entity, target) {
    ...
    // NEW: CAP-style non-draft operations
    srv.on('getAttachments', async (req) => {
      return this.getAttachments(req);
    });
    srv.on('getAttachmentContent', async (req) => {
      return this.getAttachmentContent(req);
    });
    srv.on('uploadAttachment', async (req) => {
      return this.uploadAttachment(req);
    });
    srv.on('updateAttachment', async (req) => {
      return this.updateAttachment(req);
    });
    srv.on('deleteAttachment', async (req) => {
      return this.deleteAttachment(req);
    });
    srv.on('activateDraft', async (req) => {
      return this.activateDraft(req);
    });
    srv.on('discardDraft', async (req) => {
      return this.discardDraft(req);
    });
    srv.on('bulkDelete', async (req) => {
      return this.bulkDelete(req);
    });
    srv.on('getStatistics', async (req) => {
      return this.getStatistics(req);
    });
  }

  // NEW: Active attachment handlers
  async handleActiveAttachments(req) {
    const { repositoryId } = getConfigurations();
    const attachmentsEntity = cds.model.definitions[req.target.name];
    const entityId = req.data.entityId;
    
    return await getActiveAttachments(attachmentsEntity, entityId, repositoryId);
  }

  async handleCreateActiveAttachment(req) {
    const { repositoryId } = getConfigurations();
    const attachmentsEntity = cds.model.definitions[req.target.name];
    ...
    return await createActiveAttachment(attachmentsEntity, attachmentData);
  }

  async handleUpdateActiveAttachment(req) {
    const attachmentId = req.req.url.match(attachmentIDRegex)[1];
    const attachmentsEntity = cds.model.definitions[req.target.name];
    
    return await updateActiveAttachment(attachmentsEntity, attachmentId, req.data);
  }

  async handleDeleteActiveAttachment(req) {
    const attachmentId = req.req.url.match(attachmentIDRegex)[1];
    const attachmentsEntity = cds.model.definitions[req.target.name];
    ...
    // Delete from SDM first
    const attachment = await getActiveAttachmentById(attachmentId, attachmentsEntity);
    if (attachment?.url) {
      await deleteAttachmentsOfFolder(this.creds, token, attachment.url);
    }

    // Then delete from database
    return await deleteActiveAttachment(attachmentsEntity, attachmentId);
  }

  async handlePromoteDraftToActive(req) {
    const attachmentId = req.data.attachmentId;
    const draftAttachments = cds.model.definitions[req.target.name + ".drafts"];
    const activeAttachments = cds.model.definitions[req.target.name];

    return await moveAttachmentToActive(draftAttachments, activeAttachments, attachmentId);
  }

  async handleGetAttachmentStatistics(req) {
    const { repositoryId } = getConfigurations();
    const attachmentsEntity = cds.model.definitions[req.target.name];
    const entityId = req.data.entityId;
  ...
  async handleBulkDeleteActiveAttachments(req) {
    const attachmentIds = req.data.attachmentIds;
    const attachmentsEntity = cds.model.definitions[req.target.name];

    return await bulkDeleteActiveAttachments(attachmentsEntity, attachmentIds);
  }

  async handleGetActiveAttachmentsByType(req) {
    const { repositoryId } = getConfigurations();
    const attachmentsEntity = cds.model.definitions[req.target.name];
    const { entityId, type } = req.data;

    return await getActiveAttachmentsByType(attachmentsEntity, entityId, repositoryId, type);
  }

  async handleGetActiveLinksForEntity(req) {
    const { repositoryId } = getConfigurations();
    const attachmentsEntity = cds.model.definitions[req.target.name];
    const entityId = req.data.entityId;

    return await getActiveLinksForEntity(attachmentsEntity, entityId, repositoryId);
  }

  async handleGetActiveFilesForEntity(req) {
    const { repositoryId } = getConfigurations();
    const attachmentsEntity = cds.model.definitions[req.target.name];
    const entityId = req.data.entityId;

    return await getActiveFilesForEntity(attachmentsEntity, entityId, repositoryId);
  }
};
