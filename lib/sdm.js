...
return duplicateFilesErrMsg;
  ...
  if (req?.attachmentsToDelete?.length>0) {
    const deletePromises = req.attachmentsToDelete.map(async (attachment) => {
      ...
      await this.handleRequest(deleteAttachmentResponse,req,attachment.url);
    });
    ...
    req.data.attachments =data;
     ...
     async handleRequest(response,req,objectId) {
      switch(response.response.status) {
        case 404:
        case 200:
          break;
        default:
          req.error(response.status,response.response.data.message)
          break;
      }
    }
