...
module.exports = class SDMAttachmentsService extends (
  ...
  async setRepository(req) {
    const attachments = cds.model.definitions[req.query.target.name];
    const { repositoryId } = getConfigurations(); 
    let nullAttachmentIds = await getAttachmentIdsWithNullRepositoryId(attachments);

    for (let i = nullAttachmentIds.length - 1; i >= 0; i--) {
      try {
        await this.get(attachments, { ID: nullAttachmentIds[i] }, req); 
      } catch (error) {
        nullAttachmentIds.splice(i, 1); 
      }
    }

    await setRepositoryId(attachments, nullAttachmentIds, repositoryId);
  }
