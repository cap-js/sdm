...
const {
  ...
  getConfigurations,
  getClientCredentialsToken,buildClientCredentialsDestination
} = require("./util/index");
const {
  getDraftAttachments,
...
const { getDestinationFromServiceBinding,retrieveJwt,jwtBearerToken} = require('@sap-cloud-sdk/connectivity');
...
async  transformSDMServiceBindingToClientCredentialsDestination(service,options,userJwt) {
    // Assuming serviceToken returns a promise that resolves with a token
    const transformedService = {
      ...
      // Assuming buildClientCredentialsDestination is a function that takes these arguments
...
getSdmInstanceName(data) {
  ...
  async onCreate(data, credentials, token, req, parentId) {
    let fileNames = [];
    const { repositoryId } = getConfigurations();
   const instance_name  = this.getSdmInstanceName(process.env.VCAP_SERVICES);
     ...
     const userJwt = await retrieveJwt(req);
        const destination = await getDestinationFromServiceBinding({
          destinationName: instance_name,
          jwt: userJwt,
          useCache: true,
          serviceBindingTransformFn: (serviceBinding, options) => this.transformSDMServiceBindingToClientCredentialsDestination(serviceBinding, options, userJwt)
        });
    await Promise.all(
      data.map(async (d) => {
