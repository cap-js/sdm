...
const {
  ...
  lockAttachmentErr,
  unlockAttachmentErr,
  attachmentAlreadyLockedErr,
  attachmentNotLockedErr
...
let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async _updateAttachments(req, context) {
    ...
    // Nothing changed — skip validation and lock check entirely
    ...
    // Check if attachment is locked — skip update with warning
    // First check the attachment object directly (covers draft context where holdId
    // is in the drafts table but not yet in the active table during SAVE)
    // Then fall back to DB query on active entity for non-draft flows
    const locked = attachment.holdId != null || await this.isAttachmentLocked(attachmentsEntity, attachment.ID);
    if (locked) {
      console.info(`[_updateAttachments] attachment is locked, skipping update`, {
        attachmentId: attachment.ID,
        filename: attachment.filename || filenameInSDM,
        holdIdFromObject: attachment.holdId
      });
      // Revert filename and custom properties in req.data so draft-to-active copy
      // preserves the original values (same pattern as validation/update errors)
      this.replacePropertiesInAttachment(req, attachment.ID, filenameInSDM, propertiesInDB, typeProperties, compositionName);
      failedReq.push({ typeOfError: 'locked', name: attachment.filename || filenameInSDM });
      return failedReq;
    }
  ...
  handleWarning(allErrors, propertyTitles) {
    ...
    const lockedAttachments = allErrors.filter(error =>
      error.typeOfError === 'locked'
    );
    const lockedNames = lockedAttachments.map(attachment => attachment.name);
    ...
    if (lockedNames.length > 0) {
      errorResponse += attachmentLockedErr(lockedNames);
    }
  ...
  async processAttachmentDeletion(req, diffData, compositionName, attachments) {
    ...
    if (req.event === "DELETE") {
      // Remove holds on locked attachments before entity deletion
      // so the DMS folder can be deleted without "Conflict" errors
      await this.removeHoldsForEntityDeletion(req, attachments);
  ...
  async addAttachmentsToDeleteList(req, deletedAttachments, attachments) {
    const attachmentsData = await getURLsToDeleteFromAttachments(
      deletedAttachments,
      attachments
    );

    if (attachmentsData.length > 0) {
      // Separate locked attachments from unlocked ones
      const lockedAttachments = attachmentsData.filter(a => a.holdId != null);
      const unlocked = attachmentsData.filter(a => a.holdId == null);

      if (lockedAttachments.length > 0) {
        console.info(`[addAttachmentsToDeleteList] locked attachments excluded from deletion`, {
          locked: lockedAttachments.map(a => ({ ID: a.ID, filename: a.filename }))
        });
        // Store locked attachments for re-insertion after draft activation
        if (!req.lockedAttachmentsToRestore) {
          req.lockedAttachmentsToRestore = [];
        }
        req.lockedAttachmentsToRestore.push(
          ...lockedAttachments.map(a => ({ ...a, target: attachments }))
        );
      }
      ...
      if (unlocked.length > 0) {
        ...
        req.attachmentsToDelete.push(...unlocked);
      }
    }
  }

  async removeHoldsForEntityDeletion(req, attachments) {
    const lockedAtts = await cds.ql.SELECT.from(attachments).where({ holdId: { '!=': null } });
    if (lockedAtts.length > 0) {
      console.info(`[removeHoldsForEntityDeletion] removing holds from ${lockedAtts.length} locked attachment(s)`);
      const destination = await this.getDestination(req);
      for (const att of lockedAtts) {
        try {
          await removeHold(this.creds, destination, att.url, att.holdId);
          console.info(`[removeHoldsForEntityDeletion] removed hold`, { ID: att.ID, filename: att.filename });
        } catch (error) {
          console.error(`[removeHoldsForEntityDeletion] failed to remove hold`, { ID: att.ID, error: error.message });
        }
      }
    }
  }
  ...
  async deleteAttachmentsWithKeys(records, req) {
    ...
    if (req?.attachmentsToDelete?.length > 0) {
       if (req?.parentId) {
        // Entity deletion — folder delete handles everything.
        // Holds were already removed in removeHoldsForEntityDeletion,
        // so skip re-insertion of locked attachments.
        req.lockedAttachmentsToRestore = [];
        const destination = await this.getDestination(req);
      ...
      if (req?.parentId) {
        // Entity deletion with all attachments locked — folder delete handles everything.
        req.lockedAttachmentsToRestore = [];
        const destination = await this.getDestination(req);
        const parentIds = Array.isArray(req.parentId) ? req.parentId : [req.parentId];
        for (const parentId of parentIds) {
    ...
    // Restore locked attachments that were excluded from deletion
    // During draft activation, CAP deletes these from the active table,
    // so we re-insert them to keep them visible
    if (req?.lockedAttachmentsToRestore?.length > 0) {
      const lockedNames = [];
      for (const lockedAtt of req.lockedAttachmentsToRestore) {
        try {
          // Fetch full attachment data before it was deleted (from the entity definition)
          // Re-insert the attachment record into the active table
          const { target, ...attachmentData } = lockedAtt;
          console.info(`[deleteAttachmentsWithKeys] restoring locked attachment`, {
            ID: attachmentData.ID,
            filename: attachmentData.filename
          });
          await cds.ql.INSERT.into(target).entries(attachmentData);
          lockedNames.push(attachmentData.filename);
        } catch (error) {
          console.error(`[deleteAttachmentsWithKeys] failed to restore locked attachment`, {
            ID: lockedAtt.ID,
            error: error.message
          });
        }
      ...
      if (lockedNames.length > 0) {
        req.warn(200, attachmentLockedErr(lockedNames));
      }
    }
  }
  ...
  async nonDraftAttachmentUpdateHandler(req) {
    ...
    if (failedReq && failedReq.length > 0) {
      ...
      } else if (error.typeOfError === 'locked') {
        req.warn(409, attachmentLockedErr([error.name]));
  ...
  async _processNonDraftAttachmentUpdate(req, attachment, attachmentsEntity, validationContext) {
    // Check if attachment is locked — skip update with warning
    const locked = await this.isAttachmentLocked(attachmentsEntity, attachment.ID);
    if (locked) {
      const filename = attachment.filename || (await this._fetchCurrentAttachment(attachmentsEntity, attachment.ID, validationContext.typeProperties))?.filename;
      return { typeOfError: 'locked', name: filename };
    }
  ...
  async attachNonDraftAttachmentDeletionData(req) {
    ...
    const attachments = await cds.ql.SELECT.from(req.subject).columns("url", "ID", "holdId", "filename");
    if (attachments.length) {
      // Block deletion of locked attachments
      const locked = attachments.filter(a => a.holdId != null);
      if (locked.length > 0) {
        return req.reject(409, attachmentLockedErr(locked.map(a => a.filename)));
      }
  ...
  /**
   * Handler for lockAttachment action.
   * Applies cmis:rm_hold on selected attachment in DMS and stores holdId in DB.
   * Each call handles one attachment — Fiori sends batch requests for multi-select.
   * @param {import('@sap/cds').Request} req - The request object
   */
  async handleLockAttachmentAction(req) {
    // Extract attachment ID from URL using regex (consistent with other actions)
    const attachmentId = req.req.url.match(attachmentIDRegex)[1];
    console.info(`[lockAttachment] action called`, {
      url: req.req.url,
      attachmentId,
    ...
    if (!attachmentId) {
      console.info(`[lockAttachment] no attachment ID extracted from URL`);
      return req.reject(400, 'No attachment selected');
    }

    // Resolve active entity name (strip .drafts if called from draft context)
    const activeEntityName = req.target.name.replace(/\.drafts$/, '');
    const isDraft = req.target.name !== activeEntityName;
    const attachmentsEntity = cds.model.definitions[activeEntityName];
    const draftEntity = isDraft ? cds.model.definitions[req.target.name] : null;
    console.info(`[lockAttachment] resolved entity`, {
      activeEntityName,
      entityFound: !!attachmentsEntity,
      isDraft,
      draftEntityFound: !!draftEntity
    });

    if (!attachmentsEntity) {
      return req.reject(404, 'Attachment entity not found');
    }

    // In draft context, try draft entity first (unsaved attachments only exist there)
    let attachment = null;
    if (draftEntity) {
      attachment = await cds.ql.SELECT.one.from(draftEntity)
        .where({ ID: attachmentId })
        .columns('ID', 'url', 'filename', 'holdId', 'mimeType');
      console.info(`[lockAttachment] draft DB lookup`, { found: !!attachment });
    }
    if (!attachment) {
      attachment = await cds.ql.SELECT.one.from(attachmentsEntity)
        .where({ ID: attachmentId })
        .columns('ID', 'url', 'filename', 'holdId', 'mimeType');
      console.info(`[lockAttachment] active DB lookup`, { found: !!attachment });
    }

    console.info(`[lockAttachment] DB lookup result`, {
      attachmentId,
      found: !!attachment,
      filename: attachment?.filename,
      holdId: attachment?.holdId,
      url: attachment?.url
    });
    ...
    if (!isDraft) {
      console.info(`[lockAttachment] rejected — entity is not in draft mode`);
      return req.reject(400, 'Lock is only allowed in draft mode. Please edit the entity first.');
    }

    if (attachment.holdId) {
      console.info(`[lockAttachment] attachment already locked`, { holdId: attachment.holdId });
      return req.reject(409, attachmentAlreadyLockedErr);
    }

    if (!attachment.url) {
      console.info(`[lockAttachment] attachment has no SDM object reference`);
      return req.reject(400, 'Attachment has no SDM object reference');
    }

    const destination = await this.getDestination(req);
    console.info(`[lockAttachment] calling applyHold in DMS`, { objectId: attachment.url });
    const status = await applyHold(this.creds, destination, attachment.url);
    console.info(`[lockAttachment] applyHold response`, { status });

    if (status === 200 || status === 201) {
      await cds.ql.UPDATE(attachmentsEntity)
        .set({ holdId: 'SDM_PLUGIN_HOLD', type: 'sap-icon://locked' })
        .where({ ID: attachmentId });
      console.info(`[lockAttachment] updated active entity holdId + icon`);

      // Also update draft entity for UI reflection during draft editing
      if (draftEntity) {
        await cds.ql.UPDATE(draftEntity)
          .set({ holdId: 'SDM_PLUGIN_HOLD', type: 'sap-icon://locked' })
          .where({ ID: attachmentId });
        console.info(`[lockAttachment] updated draft entity holdId + icon`);
      }
      console.info(`[lockAttachment] lock successful`, { filename: attachment.filename });
    } else if (status === 403) {
      console.info(`[lockAttachment] user not authorised`, { status });
      return req.reject(403, userNotAuthorisedError);
    } else {
      console.info(`[lockAttachment] unexpected status from DMS`, { status });
      return req.reject(500, lockAttachmentErr);
    }
  }

  /**
   * Handler for unlockAttachment action.
   * Removes cmis:rm_hold from selected attachment in DMS and clears holdId in DB.
   * Each call handles one attachment — Fiori sends batch requests for multi-select.
   * @param {import('@sap/cds').Request} req - The request object
   */
  async handleUnlockAttachmentAction(req) {
    // Extract attachment ID from URL using regex (consistent with other actions)
    const attachmentId = req.req.url.match(attachmentIDRegex)[1];
    console.info(`[unlockAttachment] action called`, {
      url: req.req.url,
      attachmentId,
      targetName: req.target.name,
      user: req.user?.id
    ...
    if (!attachmentId) {
      console.info(`[unlockAttachment] no attachment ID extracted from URL`);
      return req.reject(400, 'No attachment selected');
    }
    ...
    const attachmentsEntity = cds.model.definitions[activeEntityName];
    const draftEntity = isDraft ? cds.model.definitions[req.target.name] : null;
    console.info(`[unlockAttachment] resolved entity`, {
      activeEntityName,
      entityFound: !!attachmentsEntity,
      isDraft,
      draftEntityFound: !!draftEntity
    ...
    if (draftEntity) {
      attachment = await cds.ql.SELECT.one.from(draftEntity)
        .where({ ID: attachmentId })
        .columns('ID', 'url', 'filename', 'holdId', 'mimeType');
      console.info(`[unlockAttachment] draft DB lookup`, { found: !!attachment });
    }
    if (!attachment) {
      attachment = await cds.ql.SELECT.one.from(attachmentsEntity)
        .where({ ID: attachmentId })
        .columns('ID', 'url', 'filename', 'holdId', 'mimeType');
      console.info(`[unlockAttachment] active DB lookup`, { found: !!attachment });
    }

    console.info(`[unlockAttachment] DB lookup result`, {
      attachmentId,
      found: !!attachment,
      filename: attachment?.filename,
      holdId: attachment?.holdId,
    ...
    if (!isDraft) {
      console.info(`[unlockAttachment] rejected — entity is not in draft mode`);
      return req.reject(400, 'Unlock is only allowed in draft mode. Please edit the entity first.');
    }

    if (!attachment.holdId) {
      console.info(`[unlockAttachment] attachment is not locked`);
      return req.reject(409, attachmentNotLockedErr);
    }

    if (!attachment.url) {
      console.info(`[unlockAttachment] attachment has no SDM object reference`);
      return req.reject(400, 'Attachment has no SDM object reference');
    }

    const destination = await this.getDestination(req);
    console.info(`[unlockAttachment] calling removeHold in DMS`, { objectId: attachment.url });
    const status = await removeHold(this.creds, destination, attachment.url);
    console.info(`[unlockAttachment] removeHold response`, { status });

    if (status === 200 || status === 201) {
      const restoredIcon = attachment.mimeType === 'application/internet-shortcut'
        ? 'sap-icon://internet-browser'
        : 'sap-icon://document';
      await cds.ql.UPDATE(attachmentsEntity)
        .set({ holdId: null, type: restoredIcon })
        .where({ ID: attachmentId });
      console.info(`[unlockAttachment] updated active entity holdId + icon`, { icon: restoredIcon });
      ...
      if (draftEntity) {
        await cds.ql.UPDATE(draftEntity)
          .set({ holdId: null, type: restoredIcon })
          .where({ ID: attachmentId });
        console.info(`[unlockAttachment] updated draft entity holdId + icon`);
      }
      console.info(`[unlockAttachment] unlock successful`, { filename: attachment.filename });
    } else if (status === 403) {
      console.info(`[unlockAttachment] user not authorised`, { status });
      return req.reject(403, userNotAuthorisedError);
    } else {
      console.info(`[unlockAttachment] unexpected status from DMS`, { status });
      return req.reject(500, unlockAttachmentErr);
    }
  }

  /**
   * Checks if an attachment is locked (has holdId set).
   * @param {string} attachmentsEntityName - The entity name
   * @param {string} attachmentID - The attachment ID
   * @returns {Promise<boolean>} true if locked
   */
  async isAttachmentLocked(attachmentsEntityName, attachmentID) {
    const attachment = await cds.ql.SELECT.one.from(attachmentsEntityName)
      .where({ ID: attachmentID })
      .columns('holdId');
    return attachment?.holdId != null;
  }
  ...
  registerSDMHandlers(srv, entity, target) {
    ...
    // Handler for custom action 'lockAttachment'
    srv.on('lockAttachment', async (req) => {
      return this.handleLockAttachmentAction(req);
    });
    // Handler for custom action 'unlockAttachment'
    srv.on('unlockAttachment', async (req) => {
      return this.handleUnlockAttachmentAction(req);
    });
  }
};
