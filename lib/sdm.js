...
module.exports = class SDMAttachmentsService extends (
  ...
  async draftSaveHandler(req) {
    if (req?.data?.content) {
      ...
      if (attachment_val.length > 0) {
        const idToUse = getId(req);
        const attachmentToUpload = attachment_val.find(attachment => attachment.ID === idToUse);
        ...
        if (req.data.content) {
          attachment_val_create = attachment_val.filter(attachment => attachment.HasActiveEntity === false && attachment.ID ===idToUse);
        }
