...
module.exports = class SDMAttachmentsService extends (
  ...
  async init() {
    ...
    // Run orphan cleanup after all services are ready so cds.db is available
    cds.on('served', () => {
      // Slight delay to ensure DB migrations have run
      setTimeout(() => this._reconcileOrphanQueue(), 5000);
    });

    // Raise Node.js HTTP server timeouts so large file uploads (multi-GB, 10+ min)
    // are not killed by the default 300s requestTimeout.
    // headersTimeout must exceed requestTimeout to avoid a race condition.
    cds.on('listening', ({ server }) => {
      const LARGE_UPLOAD_TIMEOUT_MS = 3600000; // 1 hour — matches CF env vars
      if (server && typeof server.requestTimeout === 'number') {
        server.requestTimeout = LARGE_UPLOAD_TIMEOUT_MS;
        server.headersTimeout = LARGE_UPLOAD_TIMEOUT_MS + 5000;
      }
    });
  ...
  /**
   * On startup, retry deletion of any SDM documents that were created but
   * whose chunked upload failed and could not be cleaned up inline.
   */
  async _reconcileOrphanQueue() {
    let orphans;
    try {
      orphans = await getAllOrphans();
    } catch (err) {
      // Table may not exist yet in fresh deployments — not an error
      console.log(`[orphan-queue] Reconciliation skipped (queue not available): ${err.message}`);
      return;
    }

    if (!orphans || orphans.length === 0) {
      console.log('[orphan-queue] No orphaned documents to clean up.');
      return;
    }
    ...
    for (const orphan of orphans) {
      try {
        ...
        const cleaned = await deleteIncompleteDocumentWithRetry(
          orphan.objectId, this.creds, destination
        );
        if (cleaned) {
          await dequeueOrphan(orphan.objectId);
          console.log(`[orphan-queue] Reconciled objectId=${orphan.objectId} filename="${orphan.filename}"`);
        } else {
          console.warn(`[orphan-queue] Could not reconcile objectId=${orphan.objectId} — will retry on next startup`);
        }
      } catch (err) {
        console.error(`[orphan-queue] Unexpected error reconciling objectId=${orphan.objectId}: ${err.message}`);
      }
    }
  }
...
let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async checkVirusScanSizeLimit(req, fileSizeBytes) {
    const VIRUS_SCAN_MAX_BYTES = 400 * 1024 * 1024; // 400 MB
    if (fileSizeBytes <= VIRUS_SCAN_MAX_BYTES) return;

    const { repositoryId } = getConfigurations();
    const destination = await this.getTechnicalDestination();
    const repoInfo = await getRepositoryInfo(req, this.creds, destination);
    if (isVirusScanEnabled(repoInfo, repositoryId)) {
      req.reject(400, virusScanFileSizeErr);
    }
  }
  ...
  async draftAttachmentUploadHandler(req) {
    ...
    if (req?.data?.content) {
      ...
      await this.checkVirusScanSizeLimit(req, contentLengthNum);
      ...
      if (attachment_val.length > 0) {
        ...
        if (attachment_val_create.length > 0) {
          ...
          // Provide orphan queue callbacks so the chunked upload path can
          // record / remove incomplete documents atomically.
          attachment_val_create[0].enqueueOrphan = enqueueOrphan;
          attachment_val_create[0].dequeueOrphan = dequeueOrphan;
    ...
    await Promise.all(
      data.map(async (d) => {
        ...
        try {
          ...
          // Re-throw client-disconnect errors immediately — the request is already dead,
          // so calling req.reject() would crash the server.
          const isClientDisconnect = error.message && (
            ...
            error.message.includes('Stream is closed or aborted') ||
            error.message.includes('aborted')
          );
          if (isClientDisconnect) {
            console.warn('[onCreate] Client disconnected during upload, aborting silently', { filename: d.filename });
            return;
          }
