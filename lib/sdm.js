...
let subdomain = cds.context?.user?.authInfo?.token?.payload?.ext_attr?.zdn;
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async getParentId(attachments, req, upId){
    ...
    const folderPath = metadata?.sdmPath;
    
    let parentId = null;
    
    if (folderPath) { 
      parentId = await this.getFolderIdByDynamicPath(repositoryId, folderPath, this.creds, destination);
      return parentId ?? (await this.createNestedFolders(repositoryId, folderPath, this.creds, destination));
    }
