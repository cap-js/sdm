...
const { duplicateDraftFileErr, emptyFileErr, RenameErr, virusFileErr } = require("./util/messageConsts");
...
module.exports = class SDMAttachmentsService extends (
  ...
  async create(attachment_val_create, attachments, req, token){
    ...
    let errorResponse = "";
    failedReq.forEach((attachment) => {
      if (Object.prototype.hasOwnProperty.call(attachment, 'isVirus')) {
        attachment = virusFileErr(attachment.name);
      }
      else{
        attachment = attachment.replace("Child", RenameErr);
      }
    ...
    if (errorResponse != "") req.warn(500, errorResponse); 
  }
        ...
        if (d.content === null) {
          ...
          if (response.status == 201) {
            ...
            if(response.response.data.message == 'Malware Service Exception: Virus found in the file!'){
              failedReq.push({isVirus:true,name:d.filename})
            }
            else{
