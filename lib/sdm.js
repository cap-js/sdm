  ...
  async onCreate(data, credentials, token, attachments, req, parentId) {
    ...
    await Promise.all(
      data.map(async (d) => {
        ...
        if (d.content === null) {
          failedReq.push(
            `Content of file ${d.filename} is empty. Either it is corrupted or not uploaded properly.`
          );
          Ids.push(d.ID);
        } else {
    ...
    if (Ids.length > 0) {
      const attachmentsInFolder = await getChildrenForFolderId(
        parentId,
        token,
        credentials
      );
      if (attachmentsInFolder.length == 0) {
        await deleteAttachmentOrFolder(credentials, token, parentId);
      }
    }
