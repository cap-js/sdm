...
const { duplicateDraftFileErr, emptyFileErr,RenameErr } = require("./util/messageConsts");
const { UPSERT } = cds.ql;
...
module.exports = class SDMAttachmentsService extends (
  ...
  async create(attachment_val_create, attachments, req, token){
    ...
    if (errorResponse != "") req.error(200, errorResponse);
  }
      ...
      data.map(async (d) => {
        ...
        if (d.content === null) {
          ...
          if (response.status == 201) {
            ...
            success.push(d);
            delete d.HasActiveEntity;
            cds.tx (async ()=>{
              await UPSERT(d).into(attachments);
            })
          } else {
            Ids.push(d.ID);
