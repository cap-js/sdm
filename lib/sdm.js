...
const { duplicateDraftFileErr, emptyFileErr, renameFileErr, virusFileErr, duplicateFileErr, versionedRepositoryErr } = require("./util/messageConsts");
  ...
  async draftSaveHandler(req) {
    ...
    if (attachment_val.length > 0) {
      ...
      if(modifiedAttachments.length>0){
        errorMessage =  this.rename(modifiedAttachments, token, req)
      }
      if(attachment_val_create.length>0){
        await this.create(attachment_val_create, attachments, req, token, errorMessage)
      }
    }
  }
    ...
    await Promise.all(
      modifiedAttachments.map(async (a) => {
        ...
        if (response.status ==  undefined && response.response.status !=200) {
          ...
          // }
          failedReq.push({typeOfError:'duplicate',name:a.filename})
        }
      })
    );
