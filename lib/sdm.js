 ...
 const technicalUserDestn = await getDestinationFromServiceBinding({
  ...
  async _updateAttachments(req, context) {
    ...
    if ('note' in attachment) {
      const noteValue = attachment.note;
      if (!noteValue || !noteValue.startsWith("__BASELINE_URL__:")) {
        updatedSecondaryProperties["cmis:description"] = noteValue ?? null;
      }
    }
  ...
  async _processNonDraftAttachmentUpdate(req, attachment, attachmentsEntity, validationContext) {
    ...
    if ('note' in attachment) {
      const noteValue = attachment.note;
      if (noteValue === null || noteValue === undefined || !noteValue.startsWith("__BASELINE_URL__:")) {
        updatedSecondaryProperties["cmis:description"] = noteValue ?? null;
      }
    }
