...
module.exports = class SDMAttachmentsService extends (
  ...
  init() {
    ...
    // Temporary storage for original URLs during draft editing (for revert functionality)
  ...
  async deleteAttachmentsWithKeys(records, req) {
    ...
    const token = await fetchAccessToken(
      this.creds,
      req.user.authInfo.token.tokenValue || req.user.tokenInfo.getTokenValue()
    );
    if (req?.attachmentsToDelete?.length > 0) {
      if (req?.parentId) {
  ...
  try {
    ...
    console.error(`[editLink] unexpected error`, { error: error.message });
    throw error;
  }
}
  ...
  async updateBaselinesForEntity(attachmentsEntityName) {
    ...
    for (const [attachmentKey, baselineUrl] of this.originalUrlMap.entries()) {
      try {
        const attachment = await SELECT.one.from(attachmentsEntityName)
          .where({ 
            ID: attachmentKey,
            mimeType: "application/internet-shortcut",
        ...
        if (attachment) {
          ...
          await UPDATE(attachmentsEntityName)
            .set({ note: null })
  ...
  async handleDraftDiscardForLinks(req) {
    ...
    const draftAttachments = await SELECT.from(attachmentsEntityName)
      .where({ 
        [upKey]: parentId,
        mimeType: "application/internet-shortcut",
        note: { like: "__BASELINE_URL__:%" }
  ...
  async revertLinkInSDM(draftAttachment, originalLinkUrl, token) {
    try {
      ...
      const status = response?.status || response?.code;
      if (status === 200 || status === 201) {
        console.log(`[revertLinkInSDM] Successfully reverted link for attachment ${draftAttachment.ID}`);
      } else {
        console.error(`[revertLinkInSDM] Failed to revert link for attachment ${draftAttachment.ID}, status:`, status);
      }
    } catch (error) {
