...
return duplicateFilesErrMsg;
 ...
 let errorResponse="";
    ...
    async onCreate(data,credentials,token,attachments,req) {
     let failedReq=[],Ids =[],success =[],success_ids=[];
           var i=0;
           await Promise.all(
             data.map(async (d) => {
           ...
           let removeCondition = (obj) => Ids.includes(obj.ID);
           req.data.attachments = req.data.attachments.filter(obj => !removeCondition(obj));
           let removeSuccessAttachments = (obj) => success_ids.includes(obj.ID);

           // Filter out successful attachments
           req.data.attachments = req.data.attachments.filter(obj => !removeSuccessAttachments(obj));
