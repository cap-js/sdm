  ...
  async openAttachment(req) {
    ...
    if (response?.mimeType.toLowerCase() == "application/internet-shortcut") {
      ...
      const token = await fetchAccessToken(
        ...
        console.log("Attachment Id "+attachmentID+":"+response?.url)
         const response = await getAttachment(this.creds.uri, token, attachmentID);
       //check for sdmuser and sdmadmin roles from the token and add error message accordinlgly
      if(!checkIfSDMRolesExistInToken(decodeAccessToken(token))){
