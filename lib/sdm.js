...
const { getDestinationFromServiceBinding,retrieveJwt,transformServiceBindingToClientCredentialsDestination} = require('@sap-cloud-sdk/connectivity');
...
module.exports = class SDMAttachmentsService extends (
  ...
  async init() {
    ...
    const technicalUserDestn = await getDestinationFromServiceBinding({
      ...
      useCache: true,
      serviceBindingTransformFn: (serviceBinding, options) => transformSDMServiceBindingToClientCredentialsDestination(serviceBinding, options)
    });
    this.technicalUserDestn = technicalUserDestn;
  ...
  async checkRepositoryType(req) {
    ...
    if (repotype == undefined) {
      const repoInfo = await getRepositoryInfo(req, this.creds,this.technicalUserDestn);
...
if(response.response.data.message == 'Malware Service Exception: Virus found in the file!'){
  ...
  async handleEditLinkAction(req) {
    ...
    const response = await editLink(objectIdToUpdate, filenameToUpdate, newLinkUrl, this.creds);
