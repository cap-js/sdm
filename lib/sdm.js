...
module.exports = class SDMAttachmentsService extends (
  ...
  async get(attachments, keys) {
    ...
    // Use cds.context to get user information instead of req parameter
    const user = cds.context?.user || cds.context?.http?.req?.user;
    if (!user || !user.tokenInfo) {
      throw new Error('User authentication information not available');
    }
    const token = await fetchAccessToken(
      this.creds,
      user.tokenInfo.getTokenValue()
    );
      ...
      data.map(async (d) => {
        ...
        if (response.status == 201) {
          ...
          if (!req.target.isDraft && d.ID) {
            try {
              await DELETE.from(req.target).where({ ID: d.ID });
            } catch (cleanupError) {
          ...
          if(response.response.data.message == 'Malware Service Exception: Virus found in the file!'){
            ...
            console.error('[onCreate] Name constraint violation', { fileNames });
  ...
  async nonDraftAttachmentCreateHandler(req) {
    ...
    if (req.event === 'UPDATE' || req.event === 'PUT') {
      ...
      metadata = await SELECT.one.from(req.target)
        .where({ ID: attachmentID });
      
      if (!metadata) {
        console.error('[nonDraftAttachmentCreateHandler] Attachment not found in DB', { attachmentID });
    ...
    if (isRestrictedCharactersInName(filename)) {
      console.error('[nonDraftAttachmentCreateHandler] Restricted characters in filename', { filename });
      req.reject(409, nameConstrainErr([filename], "Upload"));
    }

    if (filename == null || filename.trim().length === 0) {
      console.error('[nonDraftAttachmentCreateHandler] Empty filename');
    ...
    const parentId = await this.getParentId(req.target, req, token);
    ...
    await this.onCreate(attachmentData, this.creds, token, req, parentId);

    // Clear content from request to avoid storing in DB
    req.data.content = null;
    // Continue to default handler to create/update DB record with metadata
  }
  ...
  async nonDraftAttachmentUpdateHandler(req) {
    ...
    const currentAttachment = await SELECT.one.from(req.target).where({ ID: attachmentID });
    
    if (!currentAttachment) {
      console.error('[nonDraftAttachmentUpdateHandler] Attachment not found', { ID: attachmentID });
    ...
    const failedReq = await this._updateAttachments(req, token, context);
    ...
    // Continue to default handler to update DB
  }
  ...
  async nonDraftEntityRenameHandler(req) {
    ...
    if (!diffData?.attachments?.length) {
      return;
    }

    // Get attachments that are being updated (not deleted, not created)
    ...
    const propertyTitles = getPropertyTitles(attachmentsEntity, updatedAttachments[0]);
    const secondaryPropertiesWithInvalidDefinitions = getSecondaryPropertiesWithInvalidDefinition(
      attachmentsEntity, updatedAttachments[0]
    );
    const secondaryTypeProperties = getSecondaryTypeProperties(attachmentsEntity, updatedAttachments[0]);

    let allErrors = [];

    for (const attachment of updatedAttachments) {
      // Get current filename from DB
      const currentAttachment = await SELECT.one.from(attachmentsEntity)
        .where({ ID: attachment.ID })
        .columns('filename', 'url', ...Object.keys(secondaryTypeProperties).map(k => k));

      if (!currentAttachment) {
        continue;
      }

      // Get properties from DB for comparison
      ...
      // Validate filename
      if (isRestrictedCharactersInName(newFilename)) {
        console.error('[nonDraftEntityRenameHandler] Restricted characters detected', { newFilename });
        allErrors.push({ typeOfError: 'restricted characters', name: newFilename });
        continue;
      }

      if (newFilename == null || newFilename.trim().length === 0) {
        console.error('[nonDraftEntityRenameHandler] Empty filename detected', { newFilename });
        allErrors.push({ typeOfError: 'empty name', name: newFilename });
        continue;
      }
      ...
      // If there are updates to make in SDM
      if (Object.keys(updatedSecondaryProperties).length > 0) {
        try {
          ...
          if (responseCode === 403) {
            console.error('[nonDraftEntityRenameHandler] No SDM roles');
            allErrors.push({ typeOfError: 'no sdm roles', name: newFilename });
          } else if (responseCode === 409) {
            console.error('[nonDraftEntityRenameHandler] Duplicate filename');
            allErrors.push({ typeOfError: 'duplicate', name: newFilename });
          } else if (responseCode === 404) {
            console.error('[nonDraftEntityRenameHandler] Attachment not found in SDM');
            allErrors.push({ typeOfError: 'not found', name: newFilename });
          }
        } catch (e) {
          console.error('[nonDraftEntityRenameHandler] Update error', {
            message: e.message,
          ...
          if (e.message.startsWith(unsupportedProperties)) {
            const unsupportedDetails = e.message.substring(unsupportedProperties.length).trim();
            allErrors.push({ typeOfError: 'unsupported properties', details: unsupportedDetails });
          } else {
            allErrors.push({ typeOfError: 'bad request', name: newFilename, message: e.message });
          }
        }
      }
    }
  ...
  async attachNonDraftAttachmentDeletionData(req) {
    ...
    const attachments = await SELECT.from(req.subject).columns("url", "ID");
