...
async function getFolderIdByDynamicPath(repositoryId, folderPath, credentials, destination) {
  const getFolderURL = `${credentials.uri}browser/${repositoryId}/root/${folderPath}?cmisselector=object`;
  try {
...
async function createNestedFolders(repositoryId, folderPath, credentials, destination) {
  const pathSegments = folderPath.split('/').filter((segment) => segment);
  let currentPath = '';
  let parentId = null;
  for (const segment of pathSegments) {
    currentPath = currentPath ? `${currentPath}/${segment}` : segment;
    const existingFolderId = await getFolderIdByDynamicPath(repositoryId, currentPath, credentials, destination);
    if (existingFolderId) parentId = existingFolderId;
    else {
      const createURL = `${credentials.uri}browser/${repositoryId}/root`;
      const formData = new FormData();
      formData.append('cmisaction', 'createFolder');
      if (parentId) formData.append('objectId', parentId);
      formData.append('propertyId[0]', 'cmis:name');
      ...
      formData.append('succinct', 'true');
      try {
        const response = await executeHttpRequest(destination, { method: 'POST', url: createURL, data: formData });
        parentId = response.data.succinctProperties['cmis:objectId'];
      } catch (error) {
        const retryFolderId = await getFolderIdByDynamicPath(repositoryId, currentPath, credentials, destination);
        if (retryFolderId) parentId = retryFolderId;
        else {
          throw error;
        }
      }
