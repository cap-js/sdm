...
async function getFolderIdByPath(req, credentials, attachments, upId, destination, compositionName) {
  const up_ = attachments.keys.up_.keys[0].$generatedFieldName;
  const { repositoryId } = getConfigurations();
  const entityId = req.data[up_] || upId;
  const folderName = compositionName ? `${entityId}_${compositionName}` : entityId;
...
async function getFolderIdByIDAsPath(req, credentials, destination, attachments, compositionName) {
  const up_ = attachments.keys.up_.keys[0].$generatedFieldName;
...
async function createFolder(req, credentials, attachments, customFolderName, destination, compositionName) {
  ...
  const baseId = req.data[upID] || customFolderName;
  const folderName = compositionName ? `${baseId}_${compositionName}` : baseId;
