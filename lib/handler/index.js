...
const { errorMessage, updateAttachmentError, unsupportedProperties, largFileVirusScanErr } = require("../util/messageConsts");
...
async function createAttachment(data, credentials, parentId, destination) {
  ...
  if (totalSize > FILE_SIZE_THRESHOLD) {
    const repoInfo = await getRepositoryInfo(null, credentials, destination);
    const isVirusScanEnabled = repoInfo?.data?.[repositoryId]?.isVirusScanEnabled;
    if (isVirusScanEnabled === 'true' || isVirusScanEnabled === true) {
      throw new Error(largFileVirusScanErr);
    }
