...
// CMIS standard secondary type properties that bypass MCM validation
const CMIS_BYPASS_PROPERTIES = ['cmis:rm_holdIds'];

// Map from bypass property to its parent secondary type
const CMIS_BYPASS_TYPE_MAP = {
  'cmis:rm_holdIds': 'cmis:rm_hold'
};
...
async function updateAttachment(
  ...
  const keysToRemove = Object.keys(updatedSecondaryProperties).filter(
    (key) => key !== "cmis:name" && !validSecondaryProperties.includes(key) && !CMIS_BYPASS_PROPERTIES.includes(key)
  );
  ...
  */
  ...
  // Ensure required secondary types for bypass properties are included
  for (const key of Object.keys(updatedSecondaryProperties)) {
    const requiredType = CMIS_BYPASS_TYPE_MAP[key];
    if (requiredType && !secondaryTypes.includes(requiredType)) {
      secondaryTypes.push(requiredType);
    }
  }
...
const DEFAULT_HOLD_ID = 'SDM_PLUGIN_HOLD';

/**
 * Applies a hold on an attachment in the DMS repository using cmis:rm_hold secondary type.
 * Once a hold is applied, the attachment becomes read-only in DMS.
 * @param {object} credentials - SDM credentials
 * @param {object} destination - HTTP destination
 * @param {string} objectId - The CMIS object ID of the attachment
 * @returns {Promise<number>} HTTP status code
 */
async function applyHold(credentials, destination, objectId) {
  const { repositoryId } = getConfigurations();
  const updateURL = `${credentials.uri}browser/${repositoryId}/root?objectId=${objectId}`;

  console.info(`[applyHold] called`, { repositoryId, objectId, updateURL });

  const formData = new FormData();
  formData.append("cmisaction", "update");
  formData.append("propertyId[0]", "cmis:secondaryObjectTypeIds");
  formData.append("propertyValue[0][0]", "cmis:rm_hold");
  formData.append("propertyId[1]", "cmis:rm_holdIds");
  formData.append("propertyValue[1][0]", DEFAULT_HOLD_ID);
  ...
  try {
    ...
    console.info(`[applyHold] DMS response`, { status: response.status });
    return response.status;
  } catch (error) {
    console.error(`[applyHold] DMS error`, {
      status: error.response?.status,
      statusText: error.response?.statusText,
...
/**
 * Removes a hold from an attachment in the DMS repository.
 * Once removed, the attachment becomes editable again.
 * @param {object} credentials - SDM credentials
 * @param {object} destination - HTTP destination
 * @param {string} objectId - The CMIS object ID of the attachment
 * @returns {Promise<number>} HTTP status code
 */
async function removeHold(credentials, destination, objectId) {
  const { repositoryId } = getConfigurations();
  const updateURL = `${credentials.uri}browser/${repositoryId}/root?objectId=${objectId}`;

  console.info(`[removeHold] called`, { repositoryId, objectId, updateURL });

  const formData = new FormData();
  formData.append("cmisaction", "update");
  formData.append("propertyId[0]", "cmis:secondaryObjectTypeIds");
  // No propertyValue[0] — removes all secondary types including cmis:rm_hold.
  // This implicitly clears cmis:rm_holdIds (setting it separately would fail
  // because removing the type removes its property definitions).
  ...
  try {
    ...
    console.info(`[removeHold] DMS response`, { status: response.status });
    return response.status;
  } catch (error) {
    console.error(`[removeHold] DMS error`, {
      status: error.response?.status,
      statusText: error.response?.statusText,
      data: error.response?.data,
      message: error.message
