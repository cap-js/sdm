const { getConfigurations,saveRepoToCache,getClientCredentialsToken } = require("../util");
...
async function setContentStream(data,
  credentials,
  token){
    const { repositoryId } = getConfigurations();
    const documentCreateURL =
    ...
    formData.append("objectId", data.url);
...
async function isRepositoryVersioned(credentials)
{
  ...
  try {
    ...
    let repoType = response.data[repositoryId].capabilities["capabilityContentStreamUpdatability"]
    if(repoType === "pwconly")
    repoType = "versioned";
  else
  repoType = "non-versioned";
  repoType =  saveRepoToCache(repositoryId,repoType);
