...
/**
 * Convert any content value (stream or buffer) to a Buffer.
 */
 ...
 *   2. Record objectId in orphan queue (written by caller via enqueueOrphan)
 *   3. Loop: ReadAheadStream.readBytes → appendContentStream
 *   4. On success: return final response (caller removes from orphan queue)
 *   5. On failure: retry-delete the incomplete document; if that also fails
 *      the orphan queue entry survives for the reconciliation job.
 */
async function uploadLargeFileInChunks(data, credentials, parentId, repositoryId, destination, totalSize) {
  ...
  try {
    ...
    const { response: emptyDocResponse, objectId: newObjectId } =
    ...
    // (the caller is responsible for enqueueOrphan/dequeueOrphan)
    if (typeof data.enqueueOrphan === 'function') {
      await data.enqueueOrphan(objectId, repositoryId, data.filename);
    }

    // Step 3 — convert content to Buffer (CDS delivers a Buffer for large bodies
    //           when busboy is used; streamToBuffer handles both cases)
    const contentBuffer = await streamToBuffer(data.content);

    readAheadStream = new ReadAheadStream(contentBuffer, totalSize, CHUNK_SIZE);
