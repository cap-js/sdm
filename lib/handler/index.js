...
async function deleteIncompleteDocumentWithRetry(objectId, credentials, destination) {
  for (let attempt = 1; attempt <= CLEANUP_MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(`[cleanup] Deleted incomplete document objectId=${objectId} on attempt ${attempt}`);
      return true;
    } catch (cleanupError) {
      const delayMs = CLEANUP_BASE_DELAY_MS * Math.pow(2, attempt - 1); // 2s, 4s, 8s
      console.error(
        `[cleanup] Attempt ${attempt}/${CLEANUP_MAX_RETRIES} failed for objectId=${objectId}: ${cleanupError.message}. ` +
...
async function uploadLargeFileInChunks(data, credentials, parentId, repositoryId, destination, totalSize) {
  ...
  try {
    ...
    if (objectId) {
      await deleteIncompleteDocumentWithRetry(objectId, credentials, destination);
    }
...
/**
 * Check whether a folder at a given path already exists in SDM.
 * Returns the CMIS objectId of the folder, or null if it doesn't exist.
 */
async function getFolderIdByDynamicPath(repositoryId, folderPath, credentials, destination) {
  const url = credentials.uri + "browser/" + repositoryId + "/root/" + folderPath + "?cmisselector=object";
  try {
...
/**
 * Walk each segment of folderPath, creating any missing folders along the way.
 * Returns the objectId of the leaf (deepest) folder.
 *
 * Concurrency safety: if a parallel upload beats us to creating a segment we
 * retry the GET after the 409 conflict — same pattern used by
 * getOrCreateCompositionFolder().
 */
async function createNestedFolders(folderPath, credentials, repositoryId, destination) {
  ...
  for (const segment of segments) {
    ...
    // Try to find the folder first — avoids redundant creates on warm paths.
    const existingId = await getFolderIdByDynamicPath(repositoryId, currentPath, credentials, destination);
    if (existingId) {
      parentId = existingId;
      continue;
    }

    // Folder doesn't exist yet — create it under /root.
    const url = credentials.uri + "browser/" + repositoryId + "/root";
    const formData = new FormData();
    formData.append("cmisaction", "createFolder");
    formData.append("propertyId[0]", "cmis:name");
    formData.append("propertyValue[0]", segment);
    formData.append("propertyId[1]", "cmis:objectTypeId");
    formData.append("propertyValue[1]", "cmis:folder");
    formData.append("succinct", "true");

    // objectId of the parent folder must be provided when creating a nested folder.
    if (parentId) {
      formData.append("objectId", parentId);
    }

    try {
      const response = await executeHttpRequest(destination, { method: 'POST', url, data: formData });
      parentId = response.data?.succinctProperties?.["cmis:objectId"];
    } catch (error) {
      if (error.response?.status === 409) {
        // Race condition — another upload created this segment concurrently.
        // Retry the GET to get the real objectId.
        const retryId = await getFolderIdByDynamicPath(repositoryId, currentPath, credentials, destination);
        if (retryId) {
          parentId = retryId;
          continue;
        }
      }
      console.error(`[createNestedFolders] Failed to create segment "${segment}" at path "${currentPath}"`, {
        status: error.response?.status,
