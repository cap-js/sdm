...
const onCreate = async req => {
  ...
  const content = req.data.attachments[0].filename;
  const contentType = req.data.attachments[0].mimetype;
  ...
  //const contentType = reqParams['cmis:contentStreamMimeType'];
  ...
  //   } else {
  //     throw new Error('cmis:name field is required.');
  //   }
  // } else {
  console.log(JSON.stringify(content));
  const result = await srv.createDocument(repositoryId, objectName, content, contentType).execute(destination);
  return convertCMISDocumentToOData(result, req.target.elements);
  //}
};
