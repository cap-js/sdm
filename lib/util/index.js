...
/**
 * Get content length from various content types
 * Supports Buffer, Stream, and objects with size property
 * @param {*} content - The content to measure
 * @returns {number} Content length in bytes, or -1 if cannot be determined
 */
function getContentLength(content) {
  ...
  // If content is a Stream with known length
  if (content.readableLength !== undefined) {
    return content.readableLength;
  }
  
  // Check if it's a Readable stream with a known size
  const { Readable } = require('stream');
  if (content instanceof Readable && content.readableLength) {
    return content.readableLength;
  }
  
  // For other stream types, try to get size from headers or metadata
  if (typeof content === 'object' && content.size) {
    return content.size;
  }
  
  // If we can't determine the size, return -1
  return -1;
}
