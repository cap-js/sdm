...
function isClientCredentialForced(req, attachmentsEntity) {
  ...
  if (attachmentsEntity)
    return attachmentsEntity[sdmAnnotationUseClientCredential] === true;
  // Direct attachment-target call (e.g. CREATE on Incidents.references).
  if (req.target?.[sdmAnnotationUseClientCredential] === true) return true;
  ...
  if (elements) {
    for (const element of Object.values(elements)) {
      if (element?.type === 'cds.Composition' && element.target) {
        ...
        if (targetDef?.[sdmAnnotationUseClientCredential] === true) return true;
      }
    }
  }
  return false;
