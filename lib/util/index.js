...
function getConfigurations() {
  const envRepositoryId = process.env.REPOSITORY_ID;
  const envRepoConfigJson = process.env.REPOSITORY_CONFIG_JSON;

  if (envRepositoryId && envRepoConfigJson) {
    try {
      const repositoryConfig = JSON.parse(envRepoConfigJson);
      return {
        repositoryId: envRepositoryId,
        repositoryConfig: repositoryConfig
      };
    } catch (error) {
      console.error("Failed to parse REPOSITORY_CONFIG_JSON from .env file. Please check if it is a valid JSON string.", error);
    }
  }
}
